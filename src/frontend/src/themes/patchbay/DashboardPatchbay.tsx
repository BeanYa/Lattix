...
function buildDemoChains(servers: Server[]): ChainView[] {
  ...
  return definitions.map((definition, chainIndex) => ({
    ...
    hops: definition.serverIndexes.map((serverIndex, hopIndex) => {
      ...
      const role = hopIndex === 0 ? 'ENTRY' : hopIndex === definition.serverIndexes.length - 1 ? 'EXIT' : 'RELAY'
      return {
  ...
  return chains
    ...
    .map((chain) => {
      ...
      return {
        ...
        metaLabel: chain.entry_port ? `${orderedHops.length} 跳 · 入口 ${chain.entry_port}` : `${orderedHops.length} 跳链路`,
...
function PatchCable({ chainTone, source, target, index }: {
  chainTone: RouteTone
  source: RouteHopView
  target: RouteHopView
  index: number
  ...
  return (
    <span
      ...
      style={{ '--pb-signal-delay': `${index * -.7}s` } as CSSProperties}
      role="img"
    ...
    >
      <svg viewBox="0 0 100 80" preserveAspectRatio="none" aria-hidden="true">
        ...
        {state.tone === 'active' ? <path className="pb-cable-signal" d={PATCH_CABLE_PATH} pathLength="100" vectorEffect="non-scaling-stroke" /> : null}
        <rect className="pb-cable-plug" x="-4" y="-2" width="8" height="18" rx="3" />
        <rect className="pb-cable-plug" x="96" y="-2" width="8" height="18" rx="3" />
        <path className="pb-cable-collar" d="M-3 5 H3 M97 5 H103" vectorEffect="non-scaling-stroke" />
      </svg>
      <span className="pb-cable-label" aria-hidden="true">{state.label}</span>
    </span>
  )
}

function RouteFocus({ chain }: { chain: ChainView }) {
  ...
  return (
    <article className={cn('pb-route-focus', `is-${chain.tone}`)} data-signal-running={signalRunning} aria-label={`选中链路：${chain.name}`}>
          ...
          {chain.hops.map((hop, index) => (
            <li key={hop.id} className={`is-${hop.tone}`}>
              <div className="pb-hop-card">
                <span className="pb-hop-sequence"><span>{String(index + 1).padStart(2, '0')}</span><b>{hop.role}</b></span>
                <span className="pb-hop-port" aria-hidden="true"><ServerIcon /><i /></span>
                ...
                <span className="pb-hop-jacks" aria-hidden="true">
                  {index > 0 ? <span className="pb-hop-jack is-in"><i /><span>IN</span></span> : <span />}
                  {chain.hops.length === 1 ? <small>单节点 · 无中继连线</small> : null}
                  {index < chain.hops.length - 1 ? <span className="pb-hop-jack is-out"><i /><span>OUT</span></span> : <span />}
                </span>
              </div>
              {index < chain.hops.length - 1 ? <PatchCable chainTone={chain.tone} source={hop} target={chain.hops[index + 1]} index={index} /> : null}
            </li>
          ))}
        </ol>
      </div>
      ...
      <footer className="pb-route-summary">
        ...
        <dl>
          <div><dt>链路名称</dt><dd>{chain.name}</dd></div>
          <div><dt>链路状态</dt><dd className={`is-${chain.tone}`}>{chain.statusLabel}</dd></div>
          <div><dt>拓扑</dt><dd>{chain.metaLabel}</dd></div>
          <div><dt>入口</dt><dd>{chain.hops[0]?.label || '--'}</dd></div>
          <div><dt>出口</dt><dd>{chain.hops.at(-1)?.label || '--'}</dd></div>
        </dl>
      </footer>
    </article>
  )
...
export default function DashboardPatchbay() {
  ...
  const issues = useMemo(() => buildIssues(displayServers, chainViews), [chainViews, displayServers])

  if (error) return <Notice tone="danger" title="控制面板连接失败">{error}</Notice>
  ...
  const selectedChain = chainViews.find((chain) => chain.id === activeChainId) ?? chainViews[0] ?? null
  ...
  const selectedServerIndex = selectedServer ? displayServers.findIndex((server) => server.id === selectedServer.id) : -1
  ...
  const chainIssues = isDemoData || chains.length === 0
  ...
  const metricItems = [
    ...
    { label: '待处理', value: issueCount, detail: systemHealthy ? '没有异常' : '资源需关注', icon: ActivityIcon },
  ]

  const runtimeItems = selectedServer ? [
    { label: 'CPU 使用率', value: selectedMetrics ? `${Math.round(selectedMetrics.cpu_percent ?? 0)}%` : '--', detail: selectedMetrics ? `LOAD ${selectedMetrics.load1.toFixed(2)}` : 'NO SIGNAL', level: selectedMetrics?.cpu_percent ?? 0, icon: CpuIcon },
    { label: '上行速率', value: formatByteRate(selectedMetrics?.network_tx_bps ?? null), detail: selectedMetrics?.network_interface || 'NO SIGNAL', level: null, icon: ArrowUpIcon },
    { label: '下行速率', value: formatByteRate(selectedMetrics?.network_rx_bps ?? null), detail: selectedMetrics?.network_interface || 'NO SIGNAL', level: null, icon: ArrowDownIcon },
    { label: '磁盘使用量', value: selectedMetrics ? humanizeBytes(selectedMetrics.disk_used) : '--', detail: selectedMetrics ? `/ ${humanizeBytes(selectedMetrics.disk_total)}` : 'NO SIGNAL', level: selectedMetrics ? percentage(selectedMetrics.disk_used, selectedMetrics.disk_total) : 0, icon: HardDriveIcon },
    { label: '运行时间', value: formatUptime(selectedMetrics?.uptime_seconds), detail: selectedServer.location || '位置待补充', level: null, icon: TimerIcon },
    { label: 'Agent 版本', value: selectedServer.agent_version || '--', detail: selectedServer.effective_xray_version ? `XRAY ${selectedServer.effective_xray_version}` : 'VERSION UNKNOWN', level: null, icon: GaugeIcon },
  ] : []
    ...
    <div className="pb-dashboard">
      <header className="pb-master-strip">
        <div className="pb-master-title">
          <span className="pb-master-index" aria-hidden="true">CH 01</span>
          <div><h1>仪表盘</h1><p>服务器信号与多跳链路总览</p></div>
        </div>
        <div className="pb-master-status">
          ...
          <span className={cn('pb-system-state', systemHealthy ? 'is-good' : 'is-warning')}>
            {systemHealthy ? <CheckCircle2Icon /> : <ActivityIcon />}{healthSummary}
          </span>
          <span className="pb-sync-time">
            {lastSyncedAt ? `${lastSyncedAt.toLocaleTimeString('zh-CN', { hour: '2-digit', minute: '2-digit' })} 已同步` : '正在同步'}
          </span>
          <button type="button" className="pb-refresh" onClick={() => void refresh()} disabled={isRefreshing} aria-busy={isRefreshing}>
            <RefreshCwIcon aria-hidden="true" />{isRefreshing ? '刷新中' : '立即刷新'}
          </button>
        </div>
      </header>
      ...
      <section className="pb-overview-strip" aria-label="核心指标">
        {metricItems.map((item, index) => (
          <article key={item.label} className={cn('pb-overview-cell', index === 3 && issueCount > 0 && 'is-warning')}>
            <item.icon aria-hidden="true" />
            <span><small>{item.label}</small><strong>{String(item.value).padStart(2, '0')}</strong></span>
            <p>{item.detail}</p>
          </article>
        ))}
      </section>
      ...
      <section className="pb-command-deck" aria-label="链路工作台">
        <div className="pb-route-bay">
          <header className="pb-route-bay-header">
            ...
            {selectedChain ? <span className={cn('pb-route-tone', `is-${selectedChain.tone}`)}><i aria-hidden="true" />{routeToneLabel(selectedChain.tone)}</span> : null}
          </header>

          <div className="pb-route-workspace">
            <aside className="pb-chain-rail" aria-label="已有链路">
              <div className="pb-chain-rail-title"><span>已有链路</span><strong>{String(chainViews.length).padStart(2, '0')}</strong></div>
              {chainViews.length ? (
                <div className="pb-chain-list">
                  {chainViews.map((chain, index) => (
                    <button key={chain.id} type="button" className={cn('pb-chain-row', chain.id === selectedChain?.id && 'is-selected', `is-${chain.tone}`)} onClick={() => setActiveChainId(chain.id)} aria-pressed={chain.id === selectedChain?.id}>
                      <span className="pb-chain-number">{String(index + 1).padStart(2, '0')}</span>
                      <span className="pb-chain-copy"><strong>{chain.name}</strong><small>{chain.metaLabel}</small></span>
                      <span className="pb-chain-state"><i aria-hidden="true" />{chain.statusLabel}</span>
                    </button>
                  ))}
                </div>
              ) : <div className="pb-chain-empty">尚未创建链路</div>}
              <Link href="/chains" className="pb-chain-all">管理全部链路 <ChevronRightIcon aria-hidden="true" /></Link>
            </aside>

            <div className="pb-route-canvas">
              {selectedChain?.hops.length ? <RouteFocus chain={selectedChain} /> : (
                <div className="pb-route-empty">
                  <RouteIcon aria-hidden="true" /><strong>等待链路拓扑</strong>
                  <span>创建链路后，可在左侧选择并检查每一跳的实时状态。</span><Link href="/chains">前往链路管理</Link>
                </div>
              )}
            </div>
          </div>
        ...
        <aside className="pb-incident-dock" aria-label="待处理事项">
          <header><div><ActivityIcon aria-hidden="true" /><h2>待处理</h2></div><strong>{String(issueCount).padStart(2, '0')}</strong></header>
          {issues.length ? (
            <div className="pb-issue-list">
              {issues.map((issue) => (
                <Link key={issue.id} href={issue.href} className={cn('pb-issue', `is-${issue.tone}`)}>
                  <span className="pb-issue-mark" aria-hidden="true" />
                  <span><small>{issue.title}</small><strong>{issue.subject}</strong><p>{issue.detail}</p></span>
                  <ChevronRightIcon aria-hidden="true" />
                </Link>
              ))}
            </div>
          ) : (
            <div className="pb-issue-clear"><CheckCircle2Icon aria-hidden="true" /><strong>当前没有异常</strong><span>服务器和链路都处于可用状态。</span></div>
          )}
          <footer><Link href="/logs/operations">查看运行日志 <ChevronRightIcon aria-hidden="true" /></Link></footer>
        </aside>
      </section>
      ...
      {selectedServer ? (
        <section className="pb-runtime-rack" aria-label="链路入口服务器运行指标">
          <header className="pb-runtime-identity">
            <span className="pb-runtime-channel">{String(selectedServerIndex + 1).padStart(2, '0')}</span>
            <div><h2>{selectedServer.alias}</h2><p>链路入口 · {channelCode(selectedServer, selectedServerIndex)} · {selectedServer.location || '位置待补充'}</p></div>
            <span className={cn('pb-runtime-state', isServerOnline(selectedServer) ? 'is-online' : 'is-offline')}>{serverConnectionLabel(selectedServer.connection_state)}</span>
          </header>
          <dl className="pb-runtime-grid">
            {runtimeItems.map((item) => (
              <div key={item.label} className="pb-instrument">
                <dt><item.icon aria-hidden="true" />{item.label}</dt><dd>{item.value}</dd>
                {item.level === null ? <span className="pb-instrument-rule" aria-hidden="true" /> : (
                  <div className="pb-meter" aria-hidden="true"><span style={{ '--pb-meter-level': `${item.level / 100}` } as CSSProperties} /></div>
                )}
