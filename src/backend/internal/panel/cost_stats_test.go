...
func seedBillingServers(t *testing.T, st *store.Store) (usdID, disabledID, cnyID int64) {
	...
	usdID, err := st.CreateServerWithPlans(ctx, store.ServerDraft{Alias: "US-LA-Direct", BootstrapToken: "token-usd", MachineType: store.MachineTypeDirect, CountryCode: "US", Location: "Los Angeles"}, &store.ServerBilling{
			Enabled: true, ProviderID: 1, AmountMinor: 1200, Currency: "USD", ServiceStartedOn: "2026-01-15", IntervalCount: 1, IntervalUnit: "month", NextRenewalOn: "2026-04-15", Status: store.BillingActive, }, store.ServerTrafficPlan{AccountingMode: "outbound", ResetAnchorOn: "2026-01-01", ResetCount: 1, ResetUnit: "month", TrackingStartedOn: "2026-01-01"})
	if err != nil {
		t.Fatal(err)
	}
	disabledID, err = st.CreateServerWithPlans(ctx, store.ServerDraft{Alias: "US-NY-Old", BootstrapToken: "token-old", MachineType: store.MachineTypeDirect, CountryCode: "US", Location: "New York"}, &store.ServerBilling{
			Enabled: false, ProviderID: 1, AmountMinor: 500, Currency: "USD", ServiceStartedOn: "2025-01-01", IntervalCount: 1, IntervalUnit: "month", NextRenewalOn: "2026-01-01", Status: store.BillingDisabled, }, store.ServerTrafficPlan{AccountingMode: "outbound", ResetAnchorOn: "2025-01-01", ResetCount: 1, ResetUnit: "month", TrackingStartedOn: "2025-01-01"})
	if err != nil {
		t.Fatal(err)
	}
	cnyID, err = st.CreateServerWithPlans(ctx, store.ServerDraft{Alias: "CN-BJ-Yearly", BootstrapToken: "token-cny", MachineType: store.MachineTypeDirect, CountryCode: "CN", Location: "Beijing"}, &store.ServerBilling{
			Enabled: true, ProviderID: 1, AmountMinor: 6000, Currency: "CNY", ServiceStartedOn: "2025-01-01", IntervalCount: 1, IntervalUnit: "year", NextRenewalOn: "2026-12-31", Status: store.BillingActive, }, store.ServerTrafficPlan{AccountingMode: "outbound", ResetAnchorOn: "2025-01-01", ResetCount: 1, ResetUnit: "month", TrackingStartedOn: "2025-01-01"})
	if err != nil {
		t.Fatal(err)
	}
...
func seedExpiredBillingServer(t *testing.T, st *store.Store) int64 {
	t.Helper()
	id, err := st.CreateServerWithPlans(context.Background(), store.ServerDraft{Alias: "HK-Expired", BootstrapToken: "token-exp", MachineType: store.MachineTypeDirect, CountryCode: "HK", Location: "Hong Kong"}, &store.ServerBilling{
			Enabled: true, ProviderID: 1, AmountMinor: 300, Currency: "CNY", ServiceStartedOn: "2025-06-01", IntervalCount: 1, IntervalUnit: "month", NextRenewalOn: "2026-02-01", Status: store.BillingExpired, }, store.ServerTrafficPlan{AccountingMode: "outbound", ResetAnchorOn: "2025-06-01", ResetCount: 1, ResetUnit: "month", TrackingStartedOn: "2025-06-01"})
	if err != nil {
		t.Fatal(err)
	}
	return id
...
func TestEstimatedBillingStatsHandlerCustomRange(t *testing.T) {
	...
	yearID, err := st.CreateServerWithPlans(ctx, store.ServerDraft{Alias: "YR-300", BootstrapToken: "token-yr300", MachineType: store.MachineTypeDirect, CountryCode: "NL", Location: "Amsterdam"}, &store.ServerBilling{
			Enabled: true, ProviderID: 1, AmountMinor: 30000, Currency: "CNY", ServiceStartedOn: "2023-01-01", IntervalCount: 1, IntervalUnit: "year", NextRenewalOn: "2030-01-01", Status: store.BillingActive, }, store.ServerTrafficPlan{AccountingMode: "outbound", ResetAnchorOn: "2023-01-01", ResetCount: 1, ResetUnit: "month", TrackingStartedOn: "2023-01-01"})
	if err != nil {
		t.Fatal(err)
	}
