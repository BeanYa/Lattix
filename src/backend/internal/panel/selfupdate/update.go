// Package selfupdate 实现面板自更新：以 GitHub release 最新版本为标准检测更新，
// 下载当前架构的单二进制面板包，按 checksums.txt 校验后原子替换自身，
// 最后向进程生命周期所有者登记重启。
//
// 进度经 GET /api/panel/update/status 轮询（阶段 + 百分比）；更新进行中
// requireAuth 拒绝其余 API 操作（423），防止用户在切换窗口内继续改动。
//
// 下载基址：默认 GitHub release（githubRepo）；环境变量 LATX_RELEASE_BASE
// 可覆盖为镜像根（e2e/运维用），布局为 <base>/<version>/<asset> + <base>/latest.txt。
// version 为 dev（非 release 构建）且无镜像基址时不可更新。
//
// 对面板侧的依赖（生命周期切换、操作日志、agent 集群更新、重启登记）经 Hooks 注入。
package selfupdate
...
// Status 是 GET /api/panel/update/status 的响应（更新进度快照）。
type Status struct {
...
// VersionInfo 是 GET /api/panel/version 的响应（更新检测结果）。
...
// Hooks 是 Updater 对面板侧的回调依赖，随构造一次性注入。
type Hooks struct {
	TransitionLifecycle    func(ctx context.Context, state, fault string, wait bool) error
	RecordOperation        func(ctx context.Context, event logging.OperationEvent) error
	EnqueueAgentUpgradeAll func(ctx context.Context, version, releaseBase string, force bool) (int, error)
	RequestRestart         func(reason string) error
}

// Updater 持有更新状态机；同时只允许一个更新流程。
type Updater struct {
	version    string
	githubRepo string
	hooks      Hooks
	...
	mu    sync.Mutex
	st    Status
	ver   VersionInfo // 最近一次检测结果缓存（status 之外给 version 端点复用）
	force bool        // 强制更新：即使版本号相同也执行覆盖安装
	wg    sync.WaitGroup
}

func New(version, githubRepo string, hooks Hooks) *Updater {
	return &Updater{version: version, githubRepo: githubRepo, hooks: hooks, st: Status{CurrentVersion: version}}
}

// releaseBase 返回下载基址根（镜像覆盖 > GitHub release download 根）。
func (u *Updater) releaseBase() string {
	...
	return "https://github.com/" + u.githubRepo + "/releases/download"
}
...
func (u *Updater) resolveLatest(ctx context.Context) (string, error) {
	if b := os.Getenv("LATX_RELEASE_BASE"); strings.TrimSpace(b) != "" {
		body, err := httpGet(ctx, u.releaseBase()+"/latest.txt")
		if err != nil {
	...
	body, err := httpGet(ctx, "https://api.github.com/repos/"+u.githubRepo+"/releases/latest")
	if err != nil {
...
func (u *Updater) CanUpdate() bool {
	return u.version != "dev" || strings.TrimSpace(os.Getenv("LATX_RELEASE_BASE")) != ""
}

// CheckVersion 以 GitHub release 最新版本检测更新（结果缓存供 version 端点复用）。
func (u *Updater) CheckVersion(ctx context.Context) (VersionInfo, error) {
	info := VersionInfo{Current: u.version, CanUpdate: u.CanUpdate()}
	if !info.CanUpdate {
	...
	info.Latest = latest
	info.UpdateAvailable = latest != u.version
...
// Begin 占据更新槽位并初始化进度状态；已在更新中时返回 ok=false。
func (u *Updater) Begin(target string, force bool) (st Status, ok bool) {
	u.mu.Lock()
	defer u.mu.Unlock()
	if u.st.Running {
		return u.st, false
...
// AbortStart 在启动前置动作（生命周期切换）失败后复位更新状态。
func (u *Updater) AbortStart(err error) {
	u.mu.Lock()
	u.st.Running = false
...
// RunAsync 后台执行更新流程，Wait 等待其结束（关停排水用）。
func (u *Updater) RunAsync(ctx context.Context) {
	u.wg.Add(1)
	go func() {
		defer u.wg.Done()
		u.run(ctx)
...
func (u *Updater) setStage(stage string, percent int, msg string) {
	u.mu.Lock()
	prev := u.st.Stage
	if prev == stage {
...
// transitionLifecycle 把面板生命周期切回指定状态（更新结束恢复 active）；未接线时空操作。
func (u *Updater) transitionLifecycle(state string) {
	if u.hooks.TransitionLifecycle != nil {
		_ = u.hooks.TransitionLifecycle(context.Background(), state, "", false)
	}
}

// recordOperation 写一条操作日志；未接线时按 nil = 关闭处理（与面板侧同规）。
func (u *Updater) recordOperation(event logging.OperationEvent) error {
	if u.hooks.RecordOperation == nil {
		return nil
	}
	return u.hooks.RecordOperation(context.Background(), event)
}
...
func (u *Updater) fail(err error) {
	...
	u.mu.Unlock()
	u.transitionLifecycle(shared.PanelStateActive)
	log.Printf("panel update: failed: %v", err)
	if logErr := u.recordOperation(logging.OperationEvent{
...
func (u *Updater) run(ctx context.Context) {
	...
	if target == st.CurrentVersion && !forceUpdate {
		...
		u.mu.Unlock()
		u.transitionLifecycle(shared.PanelStateActive)
		if err := u.recordOperation(logging.OperationEvent{
	...
	if forceUpdate {
		...
		queued, err := u.hooks.EnqueueAgentUpgradeAll(ctx, target, u.releaseBase(), true)
		if err != nil {
			os.RemoveAll(work)
	...
	if u.hooks.RequestRestart == nil {
		u.fail(errors.New("重启回调未配置"))
		return
	}
	if err := u.hooks.RequestRestart("update"); err != nil {
		u.fail(err)
	}
}
