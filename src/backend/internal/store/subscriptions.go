...
func (s *Store) SubscriptionUserIDsForChain(ctx context.Context, chainID int64) ([]int64, error) {
	return querySubscriptionUserIDs(ctx, s.db, `SELECT DISTINCT un.user_id FROM user_nodes un
		JOIN chains c ON c.service_node_id=un.node_id WHERE c.id=? ORDER BY un.user_id`, chainID)
}
...
func (s *Store) SubscriptionUserIDsForServer(ctx context.Context, serverID int64) ([]int64, error) {
	...
	rows, err := s.db.QueryContext(ctx, `SELECT r.snapshot FROM chains c
	...
	for rows.Next() {
		...
		ids, err := querySubscriptionUserIDs(ctx, s.db,
			`SELECT user_id FROM user_nodes WHERE node_id=?`, snapshot.ServiceNodeID)
		if err != nil {
			return nil, err
		}
		for _, id := range ids {
