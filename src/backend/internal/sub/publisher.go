...
func (s *Server) compileNodes(ctx context.Context, items []proxyItem, uuid string) ([]compiledNode, error) {
	...
	for _, item := range items {
		clash, err := buildProxy(item.node, item.rc, uuid)
		if err != nil {
			continue
		}
		singbox, err := buildSbOutbound(item.node, item.rc, uuid)
		if err != nil {
			continue
		}
		country := ""
		...
		out = append(out, compiledNode{
			...
			QuanX: buildQuanXLine(item.node, item.rc, uuid),
		})
	}
	return out, nil
}
...
func renderLinks(items []proxyItem, uuid string) ([]byte, error) {
	...
	for _, item := range items {
		if link, ok := buildShareLink(item.node, item.rc, uuid); ok {
