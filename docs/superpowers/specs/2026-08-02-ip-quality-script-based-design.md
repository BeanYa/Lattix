...
- 单栈机器：双栈模式下脚本对缺失家族直接跳过检测（`IPV6work=0` 时不调用 `check_IP`），stdout 仅一个 JSON 文档——正常解析，家族按 `Head.IP` 格式判定，缺失家族不出现；无任何公网 IP（stdout 无 JSON）→ `unavailable`（错误码 `no_public_address`）。
