...
- **补充（最终评审修正）**：分组用户的**直接单机节点（user_nodes）同样被遮蔽**——`assignedActiveNodes`（sub/sub.go）与 `itemsForUser`（publisher.go）的节点预过滤必须在用户属于任意分组时返回空集；`HandleSubInfo` 的快照失败回退节点数不得使用直接分配数。实现与回归测试见提交 `eafe8489b` 及 e2e「直接节点 + 直接外部订阅均被遮蔽」步骤。
...
### Task 8b: 订阅地址稳定性回归测试（sub_token / sub_url 不随分组操作变化）

**Files:**
- Modify: `src/backend/internal/store/groups_test.go`（追加生命周期稳定性用例）
- Modify: `src/backend/internal/panel/groups_test.go`（追加 sub_url 稳定性用例）
- Modify: `src/backend/internal/store/migrations_test.go`（追加存量库 sub_token 保留用例）
- Test: 上述用例本身

**Interfaces:**
- Consumes：Task 1 CRUD、Task 5 `toUserDTO`；`st.InsertUser(ctx, name, uuid, token, opts)`（签名以 `store/users_test.go` 现有用法为准）；`migrations_test.go` 的 `TestOpenMigratesLegacySchemaAndPreservesData` 建库模式（legacy schema v2）。

**背景（用户硬约束）：** 分组实现与后续补丁**不得改变已有用户的订阅地址**（`/sub/{sub_token}` 与用户页展示的 `sub_url`）。实现层面保证：分组只写关联表、读时解析，从不触碰 `users` 表；迁移只加表不迁数据。本任务用三层测试固化该不变量。

- [ ] **Step 1: 写失败测试**

store 层（追加到 `groups_test.go`）：
...
迁移层（追加到 `migrations_test.go`，复用 `TestOpenMigratesLegacySchemaAndPreservesData` 的 legacy 建库模式，版本 2 或 3 均可，须含 `users` 表带 `sub_token` 列）：
...
func TestMigrateLegacyPreservesSubToken(t *testing.T) {
	...
	defer st.Close()
	version, _ := st.SchemaVersion(ctx) // 若该方法不存在，改用 st.db.QueryRow(`PRAGMA user_version`)（同包可访问私有字段）
	if version != 12 {
...
注意：`SchemaVersion` 方法若不存在，直接用同包私有的 `st.db.QueryRowContext(ctx, "PRAGMA user_version")`（`migrations_test.go` 现有测试已用该方式，照抄其写法）；legacy 建表语句直接复制现有测试中的（含 `INSERT INTO servers/nodes` 等与否不影响本用例，但 `users` 表列必须含 `sub_token`）。

- [ ] **Step 2: 跑测试确认失败**

Run: `wsl -d Ubuntu bash -lc "cd /home/bean/workspace/Lattix-codex/.worktree/grouping && go test ./src/backend/internal/store/ -run 'TestGroupLifecyclePreservesSubToken|TestMigrateLegacyPreservesSubToken' -v 2>&1 | tail -10"` 与 panel：`go test ./src/backend/internal/panel/ -run TestSubURLStableAcrossGroupOps -v`
Expected: 编译失败（方法不存在）或断言失败——测试为本任务新引入，预期 RED 形态取决于环境；核心是**用例存在且最终必须 GREEN**（本任务的实现层早已就绪，RED 主要为验证用例有效性）。

- [ ] **Step 3: 适配编译并跑 GREEN**

本任务**不应改动业务代码**；若编译报错仅因测试辅助签名差异（如 `InsertUser` 参数、`groupsFixture` 返回值顺序），按实际签名微调用例并记录。若 GREEN 前发现业务代码确实会改变 token/sub_url（不应发生），BLOCKED 上报。

Run: `wsl -d Ubuntu bash -lc "cd /home/bean/workspace/Lattix-codex/.worktree/grouping && go test ./src/backend/internal/store/ ./src/backend/internal/panel/ 2>&1 | tail -5"`
Expected: ok（含全部既有用例）
...
```bash
git add src/backend/internal/store/groups_test.go src/backend/internal/panel/groups_test.go src/backend/internal/store/migrations_test.go
git commit -m "test: subscription URL stability across group operations"
```
...
| 8b 订阅地址稳定性 | 1, 5 | `go test ./src/backend/internal/store/ ./src/backend/internal/panel/` |
