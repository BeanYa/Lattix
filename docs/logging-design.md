...
3. 记录 API 元数据，同时避免记录请求体、凭证和敏感参数；
...
| `ip` | 操作来源 IP |
| `request_id` | 对应 HTTP 请求 ID；后台事件可为空 |
...
请求日志一行对应一个请求，每行是独立 JSON 对象：

| 字段 | 说明 |
|---|---|
| `timestamp` | UTC 时间 |
| `request_id` | 服务端生成的请求 ID，同时返回 `X-Request-ID` |
| `severity` | 请求重要程度 |
| `method` | HTTP 方法 |
| `path` | 实际请求路径；订阅 token 会哈希 |
| `route` | Go `ServeMux` 匹配的路由模板 |
| `params` | 脱敏后的 query 和 path 参数 |
| `status` | HTTP 状态码 |
| `duration_ms` | 请求处理耗时 |
...
| `error_summary` | 4xx/5xx 或 panic 的简短错误 |

请求日志不保存请求体、响应体全文、请求头、Cookie、Authorization 或会话内容。
...
- SPA 静态文件；
- `GET /api/logs/requests`，防止日志页面读取请求日志时产生自我放大。

`DELETE /api/logs/requests` 会在清空完成后由请求中间件写入，因此它会成为新的第一条请求
日志。成功的 WebSocket 升级在握手完成时立即记录为状态 `101`，不等待连接关闭。

### 4.3 请求重要程度

- `info`：1xx–3xx；
- `warning`：4xx，或处理时间超过 2 秒；
- `error`：5xx 或处理过程中发生 panic。

WebSocket `101` 始终是 `info`。
...
- 参数 JSON 总长度限制为 4096 字节，超出时添加 `_truncated=true`；
- 只在直接连接来自回环地址时信任 `X-Forwarded-For` 的首个地址，防止外部请求伪造来源 IP；
- 错误响应最多捕获前 4096 字节，仅用于提取简短错误信息。
...
| 方法 | 路由 | 说明 |
|---|---|---|
| `GET` | `/api/logs/operations` | 查询操作日志，支持过滤和分页 |
| `DELETE` | `/api/logs/operations` | 清空操作日志并保留清空审计记录 |
| `GET` | `/api/logs/requests?limit=30` | 读取最新请求日志窗口和容量状态 |
| `DELETE` | `/api/logs/requests` | 清空请求日志文件 |
