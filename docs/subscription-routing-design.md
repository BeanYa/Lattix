...
- `__LATTIX_REGION_NONE__`：无地区标识节点（「🌐 无地区」分组内部使用，模板亦可手写）。

上述选项级占位符在中间态保持原样，直到最终渲染（交付）阶段才展开为真实节点/分组名。
...
可达性增强：模板定义的分流组（地区组除外）在保留原有选项的基础上补充 DIRECT/REJECT 兜底并追加全部节点与「自动选择」「节点选择」（final）引用，使任何分流组既可指定固定节点也可回落自动选择、直连或拒绝；「自动选择」自身不补 DIRECT/REJECT（纯节点测速组，兜底出站会污染延迟测试结果），但会追加「节点选择」引用，使客户端可把自动选择手动固定到节点选择、进而强制全部分流走统一节点。节点池组（选项含 `__LATTIX_ALL__`，如自动选择/故障转移）与 final 组的选项仅保留节点与叶子分组（及 DIRECT/REJECT），剔除其他组引用——它们因此永不反向引用分流组；final 组同样被剔除组引用，故自动选择对 final 的引用不会成环，发布前另有引用环检测兜底。
...
订阅文件生成分三层：先在**中立策略层**完成全部逻辑处理（分组生成/增强/剥离/剪枝/环检测——这些环节只操作名字，不需要节点载荷；选项级占位符 `__LATTIX_ALL__`/`__LATTIX_REGIONS__`/`__LATTIX_REGION_XX__`/`__LATTIX_REGION_NONE__` 在这一层同样不展开，剪枝与环检测按占位符的解析结果判定存活性，解析为空的占位符留待最终渲染时自然消失；来源分组在这一层保持具体节点名，否则剪枝会把无选项的占位符组误判为死组），再构建**格式中间态**（节点区与来源分组区以占位符标出位置，组选项中的选项级占位符原样保留，载荷随结构在构建期捕获），最终交付前解压为真实条目并把选项级占位符展开为真实节点/分组名（clash 为含 name/type/server/port 的完整 proxy map，sing-box 为 outbound 对象，QuanX 为 server 行 / policy 行，links 为分享链接）。条目级占位符：
