...
Agent 是唯一的 Ping 发起方。认证成功后立即发送一次 WebSocket Ping，Panel 使用 Pong 原样响应；Agent 以本地单调时钟计算 RTT。初始化测试 10 秒无响应即关闭连接并进入既有重连流程，首次遥测必须等待初始化测试成功。

后续每 30 秒探测一次，最近三次有效 RTT 的中位数作为当前延迟。任一侧连续 90 秒未收到有效数据或控制帧时判定连接失效。Panel 不再主动发送 Ping，因此延迟探测与连接保活共用同一组控制帧。

Ping 载荷使用单调递增的 64 位序号。Agent 本地保存序号与发送时刻；Pong 只负责回显，Panel 与 Agent 不比较系统时钟。
...
- Agent 单元测试覆盖 CPU、负载、默认路由、网卡差值、磁盘、uptime 和 RTT 中位数；
- WebSocket 测试覆盖初始化探测、周期探测、10 秒初始化失败和 90 秒连接失效；
