#!/usr/bin/env bash
# 日志审查端到端验收（设计文档 §log）：
#   登录（含失败）→ admin.login_failed / admin.login 留痕 → 建服务器/用户/节点
#   → server.create / user.create / node.create / node.active 留痕 → 删服务器
#   → server.delete 留痕（含 alias 快照）→ GET /api/event-log 过滤/分页/详情字段正确。
...
# admin 类事件验证（login_failed + login）
ADMIN_LOG="$(api "http://$ADDR/api/event-log?category=admin")"
[[ "$(echo "$ADMIN_LOG" | py "[e['action'] for e in d['items']]")" == *"'admin.login_failed'"* ]] \
    && echo "OK: 登录失败已记 admin.login_failed" || { echo "FAIL: login_failed 未记: $ADMIN_LOG"; exit 1; }
[[ "$(echo "$ADMIN_LOG" | py "[e['action'] for e in d['items']]")" == *"'admin.login'"* ]] \
    && echo "OK: 登录成功已记 admin.login" || { echo "FAIL: login 未记: $ADMIN_LOG"; exit 1; }
# operator/ip 字段正确
[[ "$(echo "$ADMIN_LOG" | py "[e for e in d['items'] if e['action']=='admin.login'][0]['operator']")" == "admin" ]] \
...
# --- agent 上线 → agent.connect / agent.register（bootstrap 首次换发记 register）---
start_agent "$BOOTSTRAP"
sleep 1
AGENT_LOG="$(api "http://$ADDR/api/event-log?category=agent")"
AGENT_ACTIONS="$(echo "$AGENT_LOG" | py "[e['action'] for e in d['items']]")"
[[ "$AGENT_ACTIONS" == *"'agent.register'"* || "$AGENT_ACTIONS" == *"'agent.connect'"* ]] \
    && echo "OK: agent.connect/register 已记" || { echo "FAIL: agent.connect: $AGENT_LOG"; exit 1; }

# --- 建节点 → node.create + node.active（dispatcher 复写）---
api -X POST -d '{"server_id":1}' "http://$ADDR/api/nodes" >/dev/null
for _ in $(seq 1 30); do
    [[ "$(api "http://$ADDR/api/nodes" | py "d[0]['status']")" == "active" ]] && break
    sleep 1
done
NODE_LOG="$(api "http://$ADDR/api/event-log?category=node")"
[[ "$(echo "$NODE_LOG" | py "[e['action'] for e in d['items']]")" == *"'node.active'"* ]] \
    && echo "OK: node.active 已记（dispatcher 复写，保留历史）" || { echo "FAIL: node.active: $NODE_LOG"; exit 1; }
# node.create 在 admin 类
ADMIN_LOG2="$(api "http://$ADDR/api/event-log?category=admin")"
[[ "$(echo "$ADMIN_LOG2" | py "[e for e in d['items'] if e['action']=='node.create']")" != "[]" ]] \
    && echo "OK: node.create 已记（admin 类）" || { echo "FAIL: node.create"; exit 1; }

# --- 建用户 → user.create ---
api -X POST -d '{"name":"alice"}' "http://$ADDR/api/users" >/dev/null
ADMIN_LOG3="$(api "http://$ADDR/api/event-log?category=admin")"
[[ "$(echo "$ADMIN_LOG3" | py "[e for e in d['items'] if e['action']=='user.create']")" != "[]" ]] \
...
DEL_LOG="$(api "http://$ADDR/api/event-log?category=admin")"
...
# --- 关键字过滤 ---
Q_LOG="$(api "http://$ADDR/api/event-log?q=login")"
...
SID_LOG="$(api "http://$ADDR/api/event-log?server_id=1")"
[[ "$(echo "$SID_LOG" | py "d['total']")" -ge 1 ]] \
    && echo "OK: server_id 过滤返回历史事件（对象已删仍可查）" || { echo "FAIL: server_id 过滤: $SID_LOG"; exit 1; }

# --- 分页 ---
PAGE_LOG="$(api "http://$ADDR/api/event-log?limit=1&offset=0")"
